  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-ink-950 text-2xl font-bold tracking-tight">Anúncios</h1>
          ...
          <div className="divide-ink-100 divide-y">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="flex items-center gap-4 p-4">
                <div className="rounded-btn bg-ink-100 size-16 shrink-0 animate-pulse" />
                <div className="flex-1 space-y-2">
                  <div className="bg-ink-100 h-4 w-1/3 animate-pulse rounded" />
                  <div className="bg-ink-100 h-3 w-1/4 animate-pulse rounded" />
                </div>
              </div>
            ))}
          </div>
        ) : page.items.length === 0 ? (
          <p className="text-ink-500 p-12 text-center text-sm">Nenhum anúncio encontrado.</p>
        ) : (
            ...
            {page.items.map((vehicle) => (
              <li key={vehicle.id}>
                ...
                <Link
                  ...
                  className="hover:bg-ink-50 flex items-center gap-4 p-4 transition-colors"
                >
                  <div className="rounded-btn bg-ink-100 relative size-16 shrink-0 overflow-hidden">
                    {vehicle.cover_image ? (
                      <Image
                      ...
                      <span className="text-ink-400 flex h-full items-center justify-center text-[10px]">
                        sem foto
                      </span>
                    )}
                  </div>
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-ink-900 truncate font-medium">{vehicle.title}</p>
                  ...
                  <div className="hidden shrink-0 text-right sm:block">
                    <p className="text-ink-950 font-semibold">{formatPrice(vehicle.price)}</p>
                  </div>
